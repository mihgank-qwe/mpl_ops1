...
def add_aggregate_features(df: pd.DataFrame) -> pd.DataFrame:
    """Добавляет агрегированные признаки из истории платежей"""
    df = df.copy()
    ...
    # Отношение платежей к сумме счёта (деление на ноль исключено)
...
def add_age_bins(df: pd.DataFrame, age_col: str = "AGE") -> pd.DataFrame:
    """Добавляет биннинг возраста (18-25, 26-35, 36-50, 50+)"""
    df = df.copy()
...
def build_features(df: pd.DataFrame) -> pd.DataFrame:
    """Применяет все шаги создания признаков"""
